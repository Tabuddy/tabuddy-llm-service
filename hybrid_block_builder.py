...
7) CRITICAL for `block_type="experience"`: each experience `raw_text` MUST begin with the **verbatim** job header from the inputs before any narrative bullets:
   ...
   **NEVER** copy bullets or paragraphs from **Project Experiences** (e.g. lines starting with `#1`, `Project Name:`, `Client:`, `Responsibilities` under a project) into an `experience` block—even if the resume lists only short role lines under "Organizational Experiences" and longer text exists under projects. Project content belongs **only** in `block_type="project"` blocks.
