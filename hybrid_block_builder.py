...
7) CRITICAL for `block_type="experience"`: each experience `raw_text` MUST begin with the **verbatim** job header from the inputs before any narrative bullets:
   ...
   **Stacked job lines (no duties in between)**: Under **WORK EXPERIENCE**, **Organizational Experiences**, or other employment sections, if there are only **two (or more) consecutive role lines** (including compact forms like **Role@Company (from …)** and prose like **Working as … from …**), each line a full role (e.g. contains a date range) and the **very next non-blank line starts another role**—then **each** `experience` block must contain **only that one role line** (plus any bullets that appear **directly under that role** before the next role line IF ANY BULLET POINT DOESNT APPEAR BEFORE THE NEXT ROLE LINE DONT ADD ANY FROM ANYWHERE). **Stop** each block **before** the next role line and **before** the next major section heading such as **Next Role Line**, **Technical Skills**, **Skills**, **Core Competencies**, or **Project Experiences**. Do **not** append **Technical Skills** (or any skills-inventory lines) to `Experience_0` just because they follow the first role in the file; that content belongs in `skills_dump`, not `experience`.
